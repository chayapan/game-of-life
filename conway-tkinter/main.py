...
def  neighbors_of(grid_row, grid_col):
    ...
    try:
        ...
        # out-of-bound grid. we can ignore...
        pass
    return region

def apply_rules(grid_row, grid_col, propagate=True):
    ...
    if living_cells <= 4  and current_state == 1: # stay alive (rule 2)
        state[(grid_row, grid_col)] = 1
    if living_cells > 4: # to die (rule 3)
        state[(grid_row, grid_col)] = 0 
    if living_cells >= 3 and current_state == 0: # become alive (rule 4)
        state[(grid_row, grid_col)] = 1
    # need pause?
    # apply rule to neighbor (copied pointers from neighbors_of function)
    # self state[(grid_row, grid_col)]
    if propagate:
        #  propagate the rule application
        try:
            nghr = [(grid_row-1, grid_col-1), (grid_row-1, grid_col), (grid_row, grid_col-1),
                        (grid_row+1, grid_col+1), (grid_row+1, grid_col), (grid_row, grid_col+1),
                        (grid_row+1, grid_col-1), (grid_row-1, grid_col+1)
                        ]
            for n in nghr:
                apply_rules(n[0], n[1], propagate=False)
        except KeyError:
            pass
